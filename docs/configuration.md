...
| `SMOLQUERY_CLICKHOUSE_IP` / `SMOLQUERY_CLICKHOUSE_PORT` | The ClickHouse HTTP insert listener bind address and port (`127.0.0.1` / `8123`). It speaks plain HTTP, so expose it beyond the node only behind a TLS terminator. See [clickhouse.md](clickhouse.md) |
