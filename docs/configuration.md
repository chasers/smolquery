...
|---|---|
| `SMOLQUERY_ROLES` | Selects the service subtrees that start (`all`). Use `all`, or a comma-separated subset of `api,ingest,buffer,storage,query,web,pg,clickhouse,victoriametrics`. An unknown name fails the boot |
...
| `SMOLQUERY_PG_IP` / `SMOLQUERY_PG_PORT` | The Postgres wire listener bind address and port (`127.0.0.1` / `5432`). Expose the listener beyond the node only with TLS set, or behind a TLS terminator. See [postgres-wire.md](postgres-wire.md) |
| `SMOLQUERY_CLICKHOUSE_PASSWORD` | The password every ClickHouse HTTP insert and query must present (T-477), as `X-ClickHouse-Key`, basic auth, the `password` parameter, or a `Bearer` token. Unset, the edge takes the API key. A node with the `:clickhouse` role and neither refuses to boot |
| `SMOLQUERY_CLICKHOUSE_UNANSWERED_LOG` | How a statement the ClickHouse edge could not answer is logged (T-480): `redacted` (the default; string literals become `'?'`), `verbatim`, or `off`. It is counted in `smolquery_clickhouse_unanswered_total` either way. See [clickhouse.md](clickhouse.md) |
| `SMOLQUERY_CLICKHOUSE_TOTAL_ROWS_MAX_TABLES` | The most tables one statement may read `system.tables.total_rows` from (T-513, `256`). Each is one plan in the query service, two at a time, with no scan. HyperDX sums `total_rows` over every table with no `WHERE`, so set it at or above the deployment's table count; a statement over more is a 400 (code 36) rather than a sum that is short |
| `SMOLQUERY_CLICKHOUSE_CATALOG_CHECK_MS` | How often, at most, a `system.*`, `DESCRIBE`, `SHOW` or `EXISTS` statement on the ClickHouse edge reads the lake's schema version (T-529, `1000`). A new dataset, table or column is seen within it. The read is one statement; the `system` tables are rebuilt only when the version has moved |
| `SMOLQUERY_CLICKHOUSE_CATALOG_REBUILD_MS` | How long the ClickHouse edge's `system` tables may go without a rebuild when the schema version has not moved (T-529, `30000`). A clustering key moves no version, so `system.tables.sorting_key` can be this stale. A rebuild reads every table's schema and every catalog statement waits behind it: raise it on a lake of many tables. `smolquery_clickhouse_catalog_refresh_microseconds_total{result="rebuilt"}` over `smolquery_clickhouse_catalog_refreshes_total{result="rebuilt"}` is what one costs |
| `SMOLQUERY_CLICKHOUSE_IP` / `SMOLQUERY_CLICKHOUSE_PORT` | The ClickHouse HTTP listener bind address and port (`127.0.0.1` / `8123`). It speaks plain HTTP, so expose it beyond the node only behind a TLS terminator. See [clickhouse.md](clickhouse.md) |
| `SMOLQUERY_VICTORIAMETRICS_PASSWORD` | The password every Prometheus remote write must present to the VictoriaMetrics edge (PL-70), as a `Bearer` token (vmagent's `-remoteWrite.bearerToken`) or with basic auth under any user name. Unset, the edge takes the API key. A node with the `:victoriametrics` role and neither refuses to boot |
| `SMOLQUERY_VICTORIAMETRICS_TABLE` | The table every remote-written sample lands in, as `dataset.table` (`metrics.samples`). The edge creates the dataset and the table, clustered by `name, ts`, on the first write that finds them missing. Anything but two identifiers fails the boot |
| `SMOLQUERY_VICTORIAMETRICS_LOOKBACK_MS` | How far back a MetricsQL instant selector looks for a sample on the VictoriaMetrics edge (PL-70, `300000`). A query step longer than it is the lookback instead |
| `SMOLQUERY_VICTORIAMETRICS_MAX_SERIES` | The most series one MetricsQL selector, or one `/api/v1/series` request, may match on the VictoriaMetrics edge (`10000`). A query past it answers 422 instead of running |
| `SMOLQUERY_VICTORIAMETRICS_MAX_SAMPLES` | The most raw samples one MetricsQL selector may read into the node to compute its rollups (`5000000`, about 250 MB held while the query runs). A query past it answers 422 instead of running; `smolquery_victoriametrics_query_samples_total` is what to size it by |
| `SMOLQUERY_VICTORIAMETRICS_MAX_SAMPLES_PER_QUERY` | The most raw samples all the selectors of one MetricsQL query may read between them (`10000000`; `a / b + c` reads three selectors). A query past it answers 422 |
| `SMOLQUERY_VICTORIAMETRICS_MAX_QUERY_DURATION_MS` | The longest a MetricsQL query, or a metadata request, may run on the VictoriaMetrics edge, VictoriaMetrics' `-search.maxQueryDuration` (`30000`). A request's `timeout` is held to it; a query past it is cancelled and answers 503 `timeout` |
| `SMOLQUERY_VICTORIAMETRICS_MAX_POINTS_PER_SERIES` | The most points of a `query_range` step grid, VictoriaMetrics' `-search.maxPointsPerTimeseries` (`30000`; Grafana asks for about 1,000). A grid past it answers 422 |
| `SMOLQUERY_VICTORIAMETRICS_PUSHDOWN` | Whether a MetricsQL aggregate over one rollup of a selector (`sum by (job) (m[5m])`, `count(last_over_time(m[1m]))`) runs as one SQL statement in the query service instead of reading every matched sample into the node (`true`, T-568). `false` keeps every aggregate in Elixir, for a comparison |
| `SMOLQUERY_VICTORIAMETRICS_IP` / `SMOLQUERY_VICTORIAMETRICS_PORT` | The VictoriaMetrics listener bind address and port (`127.0.0.1` / `8428`, VictoriaMetrics' own). It speaks plain HTTP, so expose it beyond the node only behind a TLS terminator. Its write bodies are held to `SMOLQUERY_INSERT_MAX_NDJSON_BYTES` as sent, and counted against an in-flight limit of its own that derives as the API's does |
| `SMOLQUERY_VICTORIAMETRICS_MAX_QUERY_BYTES` | The longest MetricsQL text the VictoriaMetrics edge parses, in `query` or in each `match[]` (`16384`), VictoriaMetrics' `-search.maxQueryLen`. A longer one, or one that is not UTF-8, answers 400 `bad_data` before it is parsed |
| `SMOLQUERY_VICTORIAMETRICS_MAX_DECODED_BYTES` | The most a remote write block may inflate to on the VictoriaMetrics edge (`33554432`, 32 MiB: four times vmagent's largest default block, 8 MiB). A block past it answers 400, which vmagent drops rather than retries. Each write holds its inflated size against the edge's in-flight limit while it is decoded and written |
...
| `SMOLQUERY_STORAGE_COMPACT_MEMORY_LIMIT` | The DuckDB memory limit for the compaction engine (T-259). Compaction runs on its own engine, so a timed-out merge cannot starve seals. The compactor recycles the engine after a call exit. Unset, the limit is a quarter of the container's cgroup memory limit. Only without a cgroup limit does the engine fall back to `SMOLQUERY_MEMORY_LIMIT`. The boot log shows the resolved value and its source, and beside a configured value it shows what the quarter would have been (T-458): the sandbox ran compaction at a configured 512 MiB in a 6 Gi container, every merge of one table OOMed, and five gigabytes sat idle. Prefer leaving it unset |
| `SMOLQUERY_COMPACT_BACKOFF_BASE_MS` | The cooldown after a table's first failed compaction (`600000`, T-458). After `n` consecutive failures the table is left out of the sweep until `min(base × 2^(n-1), max)` has passed. A success clears the cooldown. The default is two sweep intervals, so the first failure costs one skipped sweep. `0` turns the cooldown off. A merge OOM while the table's row cap is above its floor, and a corruption-shaped failure, do not back off: the row-cap calibration (T-262) and the quarantine (T-310) need the next sweep. The compactor used to re-run a merge that OOMed every time on every sweep, minutes of merge per attempt, and the seals queued behind its catalog transaction timed out |
| `SMOLQUERY_COMPACT_BACKOFF_MAX_MS` | The ceiling on that cooldown (`14400000`, four hours). Must be positive; turn the cooldown off through the base |
...
| `SMOLQUERY_GIT_SHA` | The git commit this build came from, shown beside the version on the cluster page (T-465). The image build sets it from the `GIT_SHA` build argument; unset, the page shows the version alone. |
...
| `SMOLQUERY_WARM_ENGINES` | How many job engines the query service keeps bootstrapped ahead of demand (`2`, PL-50). A job engine's bootstrap — three extension loads and the catalog `ATTACH` — measured ~650 ms per query on prod; a warm engine takes it off the request path. `0` starts every engine cold. Each warm engine holds one catalog connection and DuckDB's baseline memory while it waits; it is recycled after 5 minutes and probed by the pool when built and on every 30 s tick, so a stale connection is replaced off the request path (T-548). Read `smolquery_query_engines_total{source}` to see the warm/cold split, and `smolquery_query_engine_microseconds_total{source}` over it for the mean acquire per source; that series had no label before T-548, so a dashboard on the bare name must add one |
| `SMOLQUERY_WARM_PROBE` | The statement the pool vouches for a warm engine with, when its build finishes and on every 30 s tick (T-548). A warm engine that fails it is stopped and replaced; the checkout itself does no I/O. Unset, it is `SELECT id FROM ducklake_current_snapshot(...)` — the same read the planner opens every job with, one row — when the catalog configuration names a lake, and `SELECT 1` otherwise. A deployment sets it to compare a cheap probe against the lake read without a release: `smolquery_query_engine_probe_microseconds_total{outcome}` divided by `smolquery_query_engine_probes_total{outcome}` is the mean probe, and `smolquery_query_engine_microseconds_total{source}` over `smolquery_query_engines_total{source}` the mean acquire the job still pays |
| `SMOLQUERY_HOT_PIN_MAX_AGE_MS` | How long a pinned read may live (`300000`, PL-58 layer 8). A Postgres wire `REPEATABLE READ` block pins its hot tier; a micro-segment it pinned is reaped `retire_grace_ms` after its seal, and a read past that point could come back short. A block older than this answers `72000` on its next statement. The value must sit below the buffer's `retire_grace_ms` (`600000`) by more than the cluster's clock skew |
| `SMOLQUERY_TOP_N_PROBE_ROWS` | The hot-row budget of the Top-N bound's second probe round (`1000000`, T-400). An `ORDER BY col LIMIT n` query over one table probes the newest micro-segments for the n-th value of `col` and reads only the entries that can reach it. Round 1 probes the newest entries whose rows cover `n`; round 2, when round 1 found fewer than `n` matching rows, probes the newest entries whose rows cover this many. `0` turns the bound off: every unclaimed micro-segment is read, as before |
...
| `SMOLQUERY_DISTRIBUTED_MIN_FILES` | The smallest shardable file count a query scatters over (`8`). Under it, the fixed costs — one engine start per worker, one partial transfer per shard — outweigh the scan (PL-48). File count is not the only gate: a bare `SELECT count(*) FROM t` never scatters at any size, since the footers answer it without a scan (T-448) |
| `SMOLQUERY_DISTRIBUTED_VALUE_LIST_MAX_ROWS` | The most rows a plan may read and still scatter an aggregate that ships values, not numbers (`50000000`, PL-68): `uniq`, `uniqExact`, `count(DISTINCT x)`, `argMax`, `argMin`, `groupArray`, `groupUniqArray`, `quantile`. Such a split is exact, and its partials are as large as the data is distinct, so the plan's row count is the bound known before it runs. Over it, or when the plan has no sizes, the query runs on one engine |
| `SMOLQUERY_DISTRIBUTED_ROW_PARTIAL_MAX_ROWS` | The largest `LIMIT n` a sampled subquery may have and still scatter (`10000000`, T-540): `SELECT agg FROM (SELECT cols FROM t WHERE ... LIMIT n)`, as a CTE or in FROM, which is how HyperDX fills its filters sidebar. Each shard answers its own `LIMIT n` and ships those rows, so `n` times the worker count is the most the coordinator reads. Over it, the statement runs on one engine |
