...
| `SMOLQUERY_PG_IP` / `SMOLQUERY_PG_PORT` | The Postgres wire listener bind address and port (`127.0.0.1` / `5432`). The password crosses in cleartext until TLS lands, so expose the listener only behind a TLS terminator or on a private network. See [postgres-wire.md](postgres-wire.md) |
