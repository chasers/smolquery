...
| `SMOLQUERY_COMPACT_MAX_ROWS` | cap on a compaction group's summed rows (T-260). Unset, the cap derives from the compaction engine's memory budget at **512 bytes per row**, floored at `65536` rows; without a resolvable budget it falls back to `4194304` (T-262). `compact_max_bytes` bounds compressed bytes, and on ~100x-compressible data a 47 MiB group held ~25M rows — merge cost scales with rows, so the group blew the merge's five-minute budget and re-planned identically every sweep. Sizing already reads each footer's `num_rows`, so the cap costs no new I/O. A head file no neighbor fits beside under the cap is skipped, so a row-heavy file cannot wedge the table's backlog. The cap also adapts per table at runtime: a merge OOM halves the table's cap, and sustained evidence at the tightened cap earns it back (`Smolquery.StorageService.Compactor.adjusted_row_caps/3`) |
