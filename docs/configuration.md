...
| `SMOLQUERY_PG_IDLE_TXN_TIMEOUT_MS` | Terminates a Postgres wire connection that sits idle inside a transaction block (`300000` ms; `0` disables). A block pins its snapshot (PL-58 layer 7), and an abandoned block would hold that pin; the refusal is Postgres's own `FATAL 25P03`. Settable per session with `SET idle_in_transaction_session_timeout` |
