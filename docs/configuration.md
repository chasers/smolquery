...
| `SMOLQUERY_VICTORIAMETRICS_MAX_SAMPLES` | The most raw samples one MetricsQL selector may read into the node to compute its rollups (`20000000`). A query past it answers 422 instead of running; `smolquery_victoriametrics_query_samples_total` is what to size it by |
