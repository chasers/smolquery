...
| `SMOLQUERY_CLICKHOUSE_PASSWORD` | The password every ClickHouse HTTP insert must present (T-477), as `X-ClickHouse-Key`, basic auth, the `password` parameter, or a `Bearer` token. Unset, the edge takes the API key. A node with the `:clickhouse` role and neither refuses to boot |
