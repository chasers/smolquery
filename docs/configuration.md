...
| `SMOLQUERY_DISTRIBUTED_QUERY` | Lets a decomposable aggregate query scatter its file list across several DuckDB instances (`false`, PL-49, PoC). A query that does not decompose, or any distributed failure, runs the normal single-engine path. With clustering on, the workers are the connected nodes that run the query service; without, `SMOLQUERY_DISTRIBUTED_LOCAL_WORKERS` instances run on this node |
