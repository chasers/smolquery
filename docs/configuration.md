...
| `SMOLQUERY_ALLOCATOR_BACKGROUND_THREADS` | Whether every DuckDB instance runs jemalloc's background thread (`true`, T-452). Without it DuckDB keeps the pages an encode or a merge freed — a buffer node settled 1.1–1.5 GiB above its start after a burst of encodes that `duckdb_memory()` never saw — and the container's anon memory climbs until the kill. One extra OS thread per instance; `false` turns it off |
