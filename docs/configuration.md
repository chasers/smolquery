...
| `SMOLQUERY_COMPACT_BUCKET_MS` | the time-bucket width compaction ownership shards on (`3600000`, T-269). Ownership used to be per table, so one node compacted a hot table alone while its peers idled; the ring now owns `{table, bucket}`, where a segment's bucket is its ULID timestamp over this value. Buckets are disjoint — two nodes cannot plan the same group — and a merge group never crosses a bucket boundary, so merged output stays time-local. Smaller buckets spread a backlog across more nodes but strand more boundary files under `compact_min_inputs` |
