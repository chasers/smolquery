...
| `SMOLQUERY_COMPACT_BACKOFF_BASE_MS` | The cooldown after a table's first failed compaction (`600000`, T-458). After `n` consecutive failures the table is left out of the sweep until `min(base × 2^(n-1), max)` has passed. A success clears the cooldown. The default is two sweep intervals, so the first failure costs one skipped sweep. `0` turns the cooldown off. The compactor used to re-run a merge that OOMed every time on every sweep, minutes of merge per attempt, and the seals queued behind its catalog transaction timed out |
