...
| `SMOLQUERY_COMPACT_MAX_INPUTS` | cap on segments one compaction merge reads (`12`, T-244). The byte ceiling bounds the output, not the file count — 128 MiB of small segments is over a hundred `read_parquet` inputs, and an unbounded input list outruns the engine's 30 s call timeout. Must be at least `compact_min_inputs`; a larger run compacts across sweeps |
...
| `SMOLQUERY_SEAL_BATCH_MAX_FILES` | cap on micro-segments one seal claim freezes (`12`, T-244). The seal merges a claim in one DuckDB call, so an uncapped claim grows with ingest throughput until the merge outruns the engine's 30 s call timeout — measured at 36+ inputs on the eu-central-1 sandbox — and a frozen claim retries the same oversized merge forever. A backlog larger than the cap seals in several claims, oldest first |
