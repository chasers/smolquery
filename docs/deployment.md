...
- **A catalog this build creates is DuckLake format `1.1-dev1`.** The 1.5.3 extension refuses to attach it (`DuckLake catalog version mismatch`), so a deployment whose catalog was first created by this release cannot roll the image back to 0.20.0. A catalog created by 1.5.3 stays at format `1.0`: this build attaches, reads and writes it without a migration, and 1.5.3 reads it back afterwards, so an existing deployment keeps its rollback. DuckDB 2.0's own storage format bump applies only to native DuckDB database files, which this deployment does not keep; the metadata lives in SQLite or Postgres.
