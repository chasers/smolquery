...
- **An out-of-range `epoch_ms` in a materialized column is an INTERNAL error in this build.** An execution error from a function not marked fallible invalidates the whole database rather than yielding NULL under `TRY`, so a batch carrying such a value takes the writer's engine subtree down, the group commit it was in is refused whole, and a client retrying the same bytes cycles the engine again. Nothing in the write path can catch it; the build after this one (`alpha41396`) already has the fix, and the pin moves on with the release. The writer test for the case is skipped by pinned version, so it runs again the moment the pin moves.
- **The driver mirror is now the trust root.** adbc downloads the driver by URL and verifies nothing, and the URL moved from DuckDB's own release page to the prerelease on this repository. Anyone who can replace that release's assets can replace the engine every build links. The SHA-256 of each asset is in the release's `SHA256SUMS` and in `docs/configuration.md`; compare before trusting a rebuilt image. The pin returns to DuckDB's release page with 2.0.0.
