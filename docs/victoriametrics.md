...
- **What crosses into the BEAM.** An aggregate over one rollup of a selector, `sum by (job) (rate(m[5m]))` and the other shapes `SmolqueryVictoriaMetrics.Pushdown` covers, runs as one SQL statement in the query service, and the node receives one row per group per step; no sample crosses, and `max_samples` does not apply. Everything else that reads a selector, `rate(m[5m])` on its own, `topk(3, m)`, a rollup over a subquery, is one query job per selector that copies every matched sample into the node as two lists per series, about 50 bytes a sample, and rolls it up in Elixir over what it read. That is what the ceilings above bound. What runs in Elixir above a pushed aggregate (`histogram_quantile`, a binary operator, `topk`) works over groups times steps, not samples.
