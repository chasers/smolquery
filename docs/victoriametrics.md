...
- **Tier 1 moves samples into the BEAM.** Each selector is one query job, grouped by series with its samples as two lists, and the rollups run in the node over what they read, about 50 bytes a sample. That is why the ceilings above exist. An aggregate (`sum`, `min`, `max`, `avg`, `count`, with `by` or no modifier) over one rollup of a selector (`default_rollup`, `last_over_time`, `sum_over_time`, `count_over_time`, `min_over_time`, `max_over_time`, `avg_over_time`, with at most a window) runs as one SQL statement instead, one row per output series per step, and `max_samples` does not apply to it (`SmolqueryVictoriaMetrics.Pushdown`, T-568; `SMOLQUERY_VICTORIAMETRICS_PUSHDOWN=false` turns it off). The rate family (`rate`, `increase`, `delta`, `irate` and their kin, `first_over_time`, `present_over_time`) and an `offset` on the selector push too (T-585): counter resets, the scrape-interval estimate and the sample before the window are computed per series in SQL window functions, and the aggregate reads one row per series and point. What stays in Elixir runs over the pushed result, groups times points, not samples.
...
- **Measured at cardinality** by `bench/victoriametrics_cardinality.exs`, the sealed tier, one sample a series ([results](../bench/results/victoriametrics_cardinality.md), T-583): a label matcher scans the metric at about 21 ns a sample, 2.4 s over a hundred million, whether it matches one series or a hundred, because Parquet keeps no statistics for a MAP value; filtering the same series by integer fingerprint is a fifth of that until the query projects `labels`, which decodes the map anyway. A selector matching a million series took 124 s, of which the scan was 0.4 s: grouping and copying the lists 29 s, counting them in Elixir 94 s. The label routes are 3.6 to 6.5 s at a hundred million samples. With the aggregate pushed down (T-568), the same count over a million series is 0.51 s, and over all ten million 1.4 s; at a hundred million series, 3.5 s and 20 s. A series table that takes `labels` off the samples rows (T-569) is the lever for the label routes and for what remains of the scan.
