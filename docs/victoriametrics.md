...
- **Measured at cardinality** by `bench/victoriametrics_cardinality.exs`, the sealed tier, one sample a series ([results](../bench/results/victoriametrics_cardinality.md), T-583): a label matcher scans the metric at about 21 ns a sample, 2.4 s over a hundred million, whether it matches one series or a hundred, because Parquet keeps no statistics for a MAP value; filtering the same series by integer fingerprint is a fifth of that until the query projects `labels`, which decodes the map anyway. A selector matching a million series took 124 s, of which the scan was 0.4 s: grouping and copying the lists 29 s, counting them in Elixir 94 s. The label routes are 3.6 to 6.5 s at a hundred million samples. Pushdown (T-568) is the lever for wide selectors; a series table that takes `labels` off the samples rows (T-569) is the lever for the label routes.
