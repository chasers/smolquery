...
`0A000 feature_not_supported`. There is no separate privilege for `ALTER
TABLE`: the password that can `SELECT` can change a table's columns, as the
API key can over HTTP.
