...
| `POST /v1/datasets/:ds/tables/:t/columns` | Add a column: `{"name": "country", "type": "STRING"}`. It is appended last and is always nullable — DuckLake cannot add a constrained column, and nullable is the only claim that is true of the rows that already exist; `"nullable": false` is a 422. Nothing is rewritten: a segment written before the add reads the column as `NULL`, in both tiers. A name the table has is a 409, and so is a name it *once* had: a dropped column's name is tombstoned and cannot be reused, because the sealer projects a claim's inputs onto the schema by name, and a re-add while unsealed micro-segments still carried the old column would bake its old values into a sealed file. An operator who knows a table holds no unsealed data older than the drop can clear the tombstone from the catalog's `smolquery_dropped_columns` table. Answers the table body, as `PATCH` does. |
| `DELETE /v1/datasets/:ds/tables/:t/columns/:name` | Drop a column. Refused (422) for the last column, for a clustering column, and for the retention column — clear the key or the policy with `PATCH` first. Files are not rewritten: the column stays visible to a read pinned at an earlier snapshot, and its name cannot be reused (above). An unknown column is a 404. Answers the table body. |
