...
| `POST /v1/datasets` | Create a dataset: `{"id": "analytics"}`. Two optional objects give the dataset its own metadata database and its own bucket (PL-51): `"catalog": {"host", "port", "database", "username", "password", "sslmode", "version"}` and `"storage": {"bucket", "prefix", "endpoint", "region", "url_style", "access_key_id", "secret_access_key"}`. Either left out means the deployment default. The fields are plain libpq and S3, so a Supabase project's Postgres and Storage work as-is; see [configuration](configuration.md#a-dataset-on-its-own-postgres-and-bucket). One Postgres database serves one dataset. `version` is the DuckLake format the lake is pinned to (default `SMOLQUERY_DUCKLAKE_VERSION`, `1.0`); every attach names it, so the lake is never migrated on first touch, and it changes only through an explicit per-dataset upgrade (PL-51 D7). The route is idempotent for the same settings and a 409 for a dataset that exists with different ones. Secrets are sealed with `SMOLQUERY_CREDENTIAL_KEY` before they reach the catalog; a node without that key answers 503 when a body carries one. |
| `GET /v1/datasets/:ds` | A dataset's settings, never its secrets. `catalog` and `storage` are `null` for the deployment default. |
| `PATCH /v1/datasets/:ds` | Rotate credentials only: `catalog.username`, `catalog.password`, `storage.access_key_id`, `storage.secret_access_key`. A storage key pair must arrive whole. Every other field is part of the dataset's identity and answers 400. |
