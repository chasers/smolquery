...
| `POST /v1/datasets/:ds/tables/:t/insert` | Streaming insert. The body is **`application/x-ndjson` only**: one JavaScript Object Notation (JSON) object per line, the same bytes ClickHouse takes as `JSONEachRow`. `insertId` is a query parameter. A JSON-array body is a 415. Two content types were two ingest paths. The array path measured 3-4x slower, with nothing to announce which path you got. A 200 means the buffer service has every accepted row durable and queryable. Rejected rows come back per-index in `insertErrors`; a partial failure is a 200, BigQuery-style. A full or overloaded buffer answers 429. A batch whose column ids the catalog has moved since this node cached the schema — a name dropped and added again — is refused by the buffer and retried once with the fresh schema; a second refusal is a 409 `ABORTED`, and a buffer that cannot reach the catalog to check is a 503 with `retry-after` (T-439). Its `retry-after` header says how far behind the write path is. A node with too many ingest-body bytes already in flight also answers 429, with `retry-after: 1`, before it reads the body (`SMOLQUERY_INSERT_MAX_IN_FLIGHT_BYTES`, T-245). A buffer node whose unsealed hot tier is at its ceiling answers 429 with `retry-after: 5`, naming the table and the depth (`SMOLQUERY_BACKLOG_MAX_ENTRIES`, T-457); it drains at the seal's pace, so back off rather than hammer. A body over `SMOLQUERY_INSERT_MAX_NDJSON_BYTES` (`8000000`) is a 413. Split a file into smaller bodies. Fan them out over concurrent inserts: that is the measured fast path ([benchmarks](benchmarks.md)). An optional `insertId` makes the request idempotent: a retry after a timeout or a dropped response, with the same id and the same rows, cannot double-count. Without an `insertId`, retries are at-least-once. `skipInvalidRows=false` makes a request all or nothing: when any row is refused, by the schema or by the flush, no row is written, the answer is a 200 with `insertedRows` 0 and the refused rows in `insertErrors`, and an `insertId` sent with it is not spent. During a rolling deploy, a buffer node on an older release answers that request 503 with `retry-after` rather than writing part of it (T-474). One exception: a retry that straddles a partition-count raise (`PATCH {"partitions": N}`) can hash to a different partition than the original while the writers' schema caches converge. That retry is at-least-once for the window. |
...
| `POST /v1/jobs` | The same query as an async job — an `ALTER TABLE` too ([DDL](#ddl)). The response returns the job pending. The route takes the same `explain` and `trace` options. Their output lands on `GET /v1/jobs/:id`. |
| `GET /v1/jobs/:id` | Status and stats, with `statementType` (`SELECT` or `ALTER_TABLE`) and, for DDL, the `ddl` outcome. Once the result time to live (TTL) expires, the answer comes from durable job history. |
| `GET /v1/jobs/:id/results` | Page a finished job's rows with `max_results` + `page_token`. Expired results are a 410. Unknown jobs are a 404. An explain or DDL job has no rows: 409. |
...
on a field in `POST /v1/datasets/:ds/tables`' schema, on the body of `POST .../columns`, or as `ADD COLUMN ts TIMESTAMP MATERIALIZED epoch_ms(ts_int)` in [DDL](#ddl). `GET` echoes the expression as written. The web UI's table page marks the column and shows the expression beside it. In `SELECT *`, in a `WHERE`, in a clustering key, it is a column like any other: the buffer computes the value as each batch is written, the file carries it, and the planner reads and prunes on it like the rest. The one client-visible difference is on insert: the column takes no value. The HTTP insert is NDJSON and parses nothing per row, so a key naming the column is ignored at the write; the row-validating path (`Smolquery.IngestService.Client.insert/4`) rejects such a row at its index in `insertErrors`. A row whose values the expression cannot take — `epoch_ms` of an integer past the timestamp range, say — stores `NULL` for it rather than failing the batch.
...
The expression reads each column as a query does, in every place it is evaluated: the buffer's write, a seal, a compaction, and the read that fills a row written before the column. So a `VARIANT` is a `VARIANT` there, and `host STRING MATERIALIZED attrs['host']::VARCHAR` stores `web-1` — the scalar the same expression answers in a query, so `host = attrs['host']::VARCHAR` holds — and `NULL` for a JSON `null` or a missing key (T-508). A JSON function straight over a variant (`attrs->>'host'`, `json_extract_string(attrs, '$.host')`) is a 400 at definition: it fails in a query too, and under the writer's `TRY` it would store `NULL` for every row. Cast first if you want one: `attrs::JSON->>'host'`. **A column defined over a variant before T-508 changes meaning on upgrade, and nothing converts it for you.** One over a key (`attrs['host']::VARCHAR`) stored the JSON text of the value, quotes included (`"web-1"`); files written then still carry it, since a stored value is read as stored, while every new write and every rewrite stores `web-1` — so until each file has been rewritten the column holds both, `WHERE host = 'web-1'` finds only the newer rows, and a `GROUP BY host` splits one host in two. One that passed the variant to a JSON function (`attrs->>'host'`), which the gates then accepted, now stores `NULL` for every new row, and a compaction rewrites its older values to `NULL`. In both cases drop the column and add it again — the same expression for a key cast, `attrs::JSON->>'host'` for a JSON function. The new column has a new id, so every existing file reads as the expression at once, and each rewrite then stores it. Do it after every node runs the new version: a buffer still on the old one keeps writing the old form.

A materialized column is nullable unless it is declared otherwise, and rewrites nothing when added. Declare it with `"nullable": false` beside `"materialized"`, or `ADD COLUMN ts TIMESTAMP NOT NULL MATERIALIZED epoch_ms(ts_int)` in DDL, and it never holds `NULL` (T-515). Where the expression gives nothing — a `NULL` input, a value it cannot take — depends on when the row was written (T-516). **A row inserted after the column exists is refused**, as a row with no value for a regular `nullable: false` column is: it comes back in `insertErrors` at its index, with `column ts is NOT NULL and its expression gave no value for the row`, and the other rows of the request land. **A row that was already in the table when the column was added takes its type's default**, as ClickHouse gives a column that is not `Nullable`: `0`, `''`, `false`, `1970-01-01 00:00:00`, `1970-01-01`. There is no insert to refuse for such a row, and the seal or compaction that rewrites it must not fail. So check what the expression gives for the rows you have before adding one: `SELECT count(*) FROM t WHERE <expr> IS NULL`. `GET`, `DESCRIBE` and `system.columns` then answer the plain type (`DateTime64(6)`, not `Nullable(DateTime64(6))`), which is what HyperDX needs of a timestamp column. It is part of the definition, so immutable like the expression: drop and re-add to change it. A `MAP` or a `VARIANT` has no default and stays nullable (422). A plain added column must still be nullable. **Define one only after every node runs a version that knows the declaration.** It lives in a side table an older build does not read: an older buffer or merge node sees the column as nullable and stores a real `NULL` where the expression gives nothing, in a file that carries the column, so nothing refills it until a compaction on a new node rewrites it, while `GET` and `system.columns` say the column is never `NULL`. A row that predates it reads as the expression at once: a read renders the column as `coalesce(stored, expression)` wherever a file in the read may lack it, so a query is exact the moment the column exists. Every rewrite — the seal of a micro-segment, a compaction of sealed ones — then stores the value (PL-61 L5), and once no file in a read predates the column the stored column is read plainly, which is what lets the sealed tier prune on its row-group stats again; the two never disagree, and there is no `MATERIALIZE COLUMN` to remember to run. That is why the expression must be deterministic (gate 2). A column it reads cannot be dropped while it exists (422). It is tracked by column id, so dropping it and re-using its name for a plain column leaves nothing behind.
...
```sql
ALTER TABLE analytics.events ADD [COLUMN] [IF NOT EXISTS] country STRING [[NOT NULL] MATERIALIZED expr]
...
The refusals are the column routes': 404 for a table or column that does not exist, 409 for a name the table has, 422 for the last column, a clustering column, the retention column, a column another is materialized from, or a partition; 400 for a statement the parser does not accept, or a `MATERIALIZED` expression the gates refuse ([materialized columns](#materialized-columns)); and 503 when the throwaway engine the gates run on could not start. `explain`, and bind parameters over the wire, are refused for DDL (400 / `0A000`).

Each statement is one catalog commit — two for a `MATERIALIZED` column, whose expression is recorded beside the column in the metadata database's own commit, with a compensating drop if that fails. There is no transaction around several, and a `BEGIN` block over the Postgres wire refuses it (`25001`). Dropping a table is not supported. A DDL job is not interrupted: the commit runs in the catalog's own process, so a cancel could only lose the answer, and the job settles when the catalog answers, which a commit retry bounds. A sync request whose `timeoutMs` expires first is still answered 504 (the wire, `57014`) while the `ALTER` lands anyway — re-read the table before retrying. There is no separate privilege for it: the API key that can query can change a table's columns.
...
ClickHouse's insert, `POST /?query=INSERT INTO db.table FORMAT RowBinary`, is not on this listener. The `:clickhouse` role serves it on its own port, 8123, the way the `:pg` role serves the Postgres wire. See [clickhouse.md](clickhouse.md).
