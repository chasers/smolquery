...
- **Settings.** `max_execution_time`, in seconds, bounds the query; a value past what a timer takes, a client's way of saying "no limit", is held to the longest one, about 49 days. Every other setting is accepted and ignored. `{name:Type}` parameters are not substituted.
