...
- **Dialect.** The statement is smolquery's SQL, the same the API and the Postgres wire run, with the same planner, tiers and result cap. It is **not** translated from ClickHouse SQL: `toStartOfInterval`, `ARRAY JOIN`, `LIMIT BY` and other ClickHouse-only syntax fail as the engine reports them. [clickhouse-sql-gaps.md](clickhouse-sql-gaps.md) enumerates what is missing.
