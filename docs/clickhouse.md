...
- **Settings.** `max_execution_time`, in seconds, bounds the query; a value past what a timer takes, a client's way of saying "no limit", is held to the longest one, about 49 days. Every other setting is accepted and ignored. A setting arrives as a URL parameter or in the statement's `SETTINGS name = value, ...` clause, before or after `FORMAT`; the clause wins. A `SETTINGS` clause that ends a subquery is dropped.
