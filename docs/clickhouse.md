...
- **Types.** Every column is `Nullable` except a map: `Int64`, `Float64`, `String`, `Bool`, `DateTime64(6)`, `Date32`, `Decimal(P, S)`, `Map(String, String)`. A `VARIANT` column is a `String` holding JSON. In JSON a 64-bit integer is a quoted string, as ClickHouse writes it by default. A `NULL` map answers as the empty map, since its type is not `Nullable`. A value that is not UTF-8 has its invalid bytes replaced with U+FFFD in the JSON formats; `TabSeparated` and RowBinary carry them as they are.
