...
* **it does not compete with the server for CPU.** The benchmarks under `bench/`
  boot smolquery's supervision tree inside the driver's own BEAM, so the driver's
  JSON encoding and garbage collection run on the same schedulers as the thing
  being measured. Past a few concurrent writers it stops being possible to tell
  a saturated server from a busy driver. Everything measured that way is a
  latency number wearing a throughput costume.
* **it drives more than one back end.** smolquery and ClickHouse differ here only
  by URL, headers, and which file holds the body.

What lives under `bench/` is not this and should not be replaced by it:
`pathprof.exs`, `columnar.exs` and `encodeconc.exs` are profilers. They call
`Validator.validate/2` directly, subtract stages from each other, and set
`flush_max_bytes` at runtime. None of that is visible over HTTP, and none of it
is load generation.
...
```bash
k6 run -e ROWS=3062 -e PROJECTS=1000 -e OUT=/tmp/smolquery-bodies scripts/k6/generate.js
```
...
```bash
curl -X POST localhost:4000/v1/datasets/logs/tables -H 'authorization: Bearer k6key' -H 'content-type: application/json' --data-binary @scripts/k6/schema.json
```
...
```bash
k6 run -e URL=http://127.0.0.1:4000/v1/datasets/logs/tables/otel_logs/insert -e BODY=/tmp/smolquery-bodies/columns.3062.json -e ROWS=3062 -e AUTH="Bearer k6key" -e VUS=4 -e DURATION=60s scripts/k6/insert.js
```

Row-major is the same run with one file swapped, which is the whole comparison:

```bash
k6 run -e URL=http://127.0.0.1:4000/v1/datasets/logs/tables/otel_logs/insert -e BODY=/tmp/smolquery-bodies/rows.3062.json -e ROWS=3062 -e AUTH="Bearer k6key" -e VUS=4 -e DURATION=60s scripts/k6/insert.js
```
...
Same script, same rows, a different URL and file. The server is the repo-local
pinned binary, not anything on `$PATH` — `scripts/clickhouse/` owns its
lifecycle, and its README explains why it is a host process and not a container:

```bash
./scripts/clickhouse/up.sh --reset
```

Then create the table from the DDL next to this file, which mirrors
`schema.json` column for column:
...
* **`fsync_after_insert` is off by default, and that is an asymmetry.**
  ...
  **MergeTree table setting, not a query setting**. In the query string it
  ...
  ```bash
  .cache/clickhouse/25.8/clickhouse client --query "ALTER TABLE bench.otel_logs MODIFY SETTING fsync_after_insert = 1, fsync_part_directory = 1"
  ```
...
```bash
go run scripts/k6/watch.go -match beam.smp -match 'k6 run' -duration 70s -out scripts/k6/results/last.json
```
...
`results/` holds one document per machine, named after it. Each records every
run on that machine: throughput, latency percentiles, and CPU and RSS **for the
server and for k6**, plus the asymmetries that were not controlled for.

A result without the generator's own CPU on the page is not a result — it cannot
distinguish a saturated server from a saturated machine. That is the whole
reason `watch.go` exists.
