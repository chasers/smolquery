defmodule SmolqueryClickHouse.PruneAndScatterTest do
  ...
  @shapes [
    ...
    {"SELECT sumIf(Duration, SeverityText = 'error'), avgIf(Duration, ServiceName = 'api') #{@from}",
     2, :ok},
    {"SELECT ServiceName, count() AS c #{@from} GROUP BY ServiceName HAVING c > 10", 2, :ok},
    {"SELECT DISTINCT ServiceName #{@from} LIMIT 100", 2, :ok},
    {"SELECT any(Body), argMax(Body, Timestamp), ServiceName #{@from} GROUP BY ServiceName", 2,
     :ok},
    {"SELECT groupUniqArray(20)(ServiceName) #{@from}", 2, :ok},
    {"SELECT groupArray(5)(Body), ServiceName #{@from} GROUP BY ServiceName", 2, :ok},
